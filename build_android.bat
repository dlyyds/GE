...
:: 2. 清掉旧 APK
:: ==========================================
:: 只在"刚收过资产"时删：资产体积缩小时 AGP 不回收旧空间、也不截断文件，
:: 会留下中央目录根本不索引的孤儿字节（实测踩过：48MB 的 skybox 换掉 192MB 的之后，
:: APK 仍是 245MB，其中 144MB 是孤儿）。资产没动（nopack）则体积不可能缩小，增量安全。
if "%PACK%"=="1" (
    if exist "%APK%" (
...
::     目录**（environments/_default_cube/ 整目录被丢，而 APK 照样构建成功，只有解包
...
echo   nopack     - 跳过 gepack（资产没动时用；此时也不删旧 APK，增量更安全）
