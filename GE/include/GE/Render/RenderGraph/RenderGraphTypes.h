...
// ============================================================================
// 外部图像在帧首的实际状态（供布局转换起点匹配）
// ============================================================================

/// 外部图像在帧首的布局状态。与 vk::ImageLayout 不同，它表达「本帧开始时
/// 图像停在哪个语义状态」，由图翻译成具体布局。
enum class ExternalImageState {
    Undefined,     ///< 帧首布局 Undefined（内容可丢弃，仅关心本次写入结果）
    Preserve,      ///< 帧首需保留上次内容，图期望布局为已知（创建导入的 ImageViewResource 见下）
    Present,       ///< 帧首处于呈现布局 ePresentSrcKHR（swapchain 图被外部转回呈现）
};
