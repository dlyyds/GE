...
public:
    ...
    /**
     * @brief 设置材质资产源文件（**已解析的绝对路径**，语义同 Texture::m_FilePath）。
     *
     * 只有从 `.gemat` 加载、或另存为 `.gemat` 之后才有值。非空 = 文件背书，
     * 序列化场景时实体覆写写引用而非内联；空 = 临时材质（网格自带材质、场景内联
     * 去重材质、编辑器里刚建还没落盘的）。写入场景文件前会归一为相对资源根的
     * 规范形（见 MaterialSerializer::CanonicalAssetRef）。
     */
