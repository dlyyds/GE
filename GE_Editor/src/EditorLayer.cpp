...
void EditorLayer::OnAttach() {
...

    {
        ...
        //serializer.Deserialize("assets/scenes/test3d.GE");
    }
...
void EditorLayer::OpenScene() {
    std::string filepath = FileDialogs::OpenFile("GE Scene (*.GE)\0*.GE\0");
    if (!filepath.empty()) {
        OpenScene(filepath);
    }
}
...
void EditorLayer::OpenScene(const std::filesystem::path &filepath) {
    ...
    if (!serializer.Deserialize(filepath.string())) {
        GE_CORE_WARN("Failed to deserialize GE Scene : {0}", filepath.string());
    }
}

void EditorLayer::SaveSceneAs() {
    const std::string filepath = FileDialogs::SaveFile("GE Scene (*.GE)\0*.GE\0");
    if (!filepath.empty()) {
        SceneSerializer serializer(m_ActiveScene);
