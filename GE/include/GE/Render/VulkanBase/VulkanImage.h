...
public:
    VulkanImageBuilder(vk::Extent3D const &extent) : Parent(vk::ImageCreateInfo{.imageType = vk::ImageType::e2D, .format = vk::Format::eR8G8B8A8Unorm,
                                                                                .extent = extent, .mipLevels = 1, .arrayLayers = 1}) {
    }

    VulkanImageBuilder(vk::Extent2D const &extent) : VulkanImageBuilder(vk::Extent3D{extent.width, extent.height, 1}) {
    }

    VulkanImageBuilder(uint32_t width, uint32_t height = 1, uint32_t depth = 1) : VulkanImageBuilder(vk::Extent3D{width, height, depth}) {
    }
