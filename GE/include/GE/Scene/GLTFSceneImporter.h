 ...
 * - node 归本导入器：先建一个以文件名命名的导入根实体，再逐 node 建实体，
 *   填 Transform（TRS/矩阵 → Translation/quat/Scale），经 Scene::SetParent 保留层级
 *   （所有 glTF 顶层 node 统一挂到根实体下），有 mesh 的 node 挂 MeshRendererComponent。
...
public:
     ...
     *
     * 保留 node 层级与变换：先建导入根实体（以文件名命名、单位变换），node 名称 →
     * TagComponent、变换 → TransformComponent、父子关系 → Scene::SetParent（顶层 node
     * 挂到导入根实体下）、有 mesh 的 node → MeshRendererComponent。
